...
export async function saveFormSubmission(formData: FormData): Promise<{ data?: FormSubmission; error?: any }> {
  try {
    const submission: FormSubmission = {
      ...
      rooms: JSON.stringify(formData.rooms || []),
      calculation_results: JSON.stringify(formData.calculationResults || {}),
    }
    ...
    if (insertedRecord) {
      ...
      const parsedRecord = {
        ...
        rooms: insertedRecord.rooms ? JSON.parse(insertedRecord.rooms) : null,
        calculation_results: insertedRecord.calculation_results ? JSON.parse(insertedRecord.calculation_results) : null
      }
